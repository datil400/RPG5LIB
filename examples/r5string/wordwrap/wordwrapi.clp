...
/*  ================================================================ */
   ...
   DCL        VAR(&PKG)    TYPE(*CHAR) LEN(10) VALUE('WORDWRAP')
...
/*  Verificar versión del sistema operativo                          */
   ...
   CHKOBJ     OBJ(R5UTILS) OBJTYPE(*SRVPGM) AUT(*USE)
   CHKOBJ     OBJ(R5EXCMGR) OBJTYPE(*SRVPGM) AUT(*USE)
   CHKOBJ     OBJ(R5STRING) OBJTYPE(*SRVPGM) AUT(*USE)
...
/*  ---------------------------------------------------------------  */
   ...
   CRTBNDRPG  PGM(&LIB/WORDWRAP1) SRCFILE(&LIB/&RPGLESRC) +
              DBGVIEW(&DBG)
   CRTBNDRPG  PGM(&LIB/WORDWRAP2) SRCFILE(&LIB/&RPGLESRC) +
              DBGVIEW(&DBG)

   SNDPGMMSG  MSG('Se ha creado el prgrama externo para UDTF WORD_WRAP.') +
              MSGTYPE(*COMP)
