...
/*  ================================================================ */
   ...
   DCL        VAR(&MINVER) TYPE(*CHAR) LEN(6) VALUE('V7R2M0')
   DCL        VAR(&PKG)    TYPE(*CHAR) LEN(10) VALUE('R5EXCMGR')
...
/*  ================================================================ */
   ...
   CRTBNDDIR  BNDDIR(&LIB/RPG5LIB) AUT(*USE) TEXT('RPG5LIB +
              Binding directory')
   MONMSG     MSGID(CPF0000)

   ADDBNDDIRE BNDDIR(&LIB/RPG5LIB) OBJ((R5EXCMGR))
   MONMSG     MSGID(CPF0000)
...
/*  ---------------------------------------------------------------  */
             ...
             CRTRPGMOD  MODULE(QTEMP/EXCEPTION) +
                          SRCFILE(&LIB/&RPGLESRC) DBGVIEW(&DBG)
             CRTRPGMOD  MODULE(QTEMP/EXCMGR) +
                          SRCFILE(&LIB/&RPGLESRC) DBGVIEW(&DBG)
             CRTRPGMOD  MODULE(QTEMP/CALLSTACK) +
                          SRCFILE(&LIB/&RPGLESRC) DBGVIEW(&DBG)
             CRTRPGMOD  MODULE(QTEMP/CALLSTKE) +
                          SRCFILE(&LIB/&RPGLESRC) DBGVIEW(&DBG)
             CRTRPGMOD  MODULE(QTEMP/UTILITIES) +
                          SRCFILE(&LIB/&RPGLESRC) DBGVIEW(&DBG)
             CRTRPGMOD  MODULE(QTEMP/RCVM0300) +
                          SRCFILE(&LIB/&RPGLESRC) DBGVIEW(&DBG)
             CRTSRVPGM  SRVPGM(&LIB/R5EXCMGR) MODULE(QTEMP/EXCEPTION +
                          QTEMP/EXCMGR QTEMP/CALLSTACK +
                          QTEMP/CALLSTKE QTEMP/UTILITIES +
                          QTEMP/RCVM0300) SRCFILE(&LIB/&SRVSRC) +
                          SRCMBR(R5EXCMGRB) TEXT('RPG5LIB Exception +
                          manager') BNDSRVPGM((R5UTILS)) +
                          ACTGRP(RPG5LIB)

   SNDPGMMSG  MSG('Se ha creado el programa de servicio R5EXCMGR.') +
              MSGTYPE(*COMP)
