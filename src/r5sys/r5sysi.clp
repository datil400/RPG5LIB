...
/*  ================================================================ */
   ...
   DCL        VAR(&PKG)    TYPE(*CHAR) LEN(10) VALUE('R5SYS')
...
/*  ---------------------------------------------------------------  */
   ...
   CRTRPGMOD  MODULE(QTEMP/OBJMAN) SRCFILE(&LIB/&RPGLESRC) +
              DBGVIEW(&DBG)
   CRTRPGMOD  MODULE(QTEMP/SYSVAL) SRCFILE(&LIB/&RPGLESRC) +
              DBGVIEW(&DBG)
   RTVMBRD    FILE(&LIB/&RPGLESRC) MBR(R5SYSB) TEXT(&TEXT)
   CRTSRVPGM  SRVPGM(&LIB/R5SYS) MODULE(QTEMP/OBJMAN QTEMP/SYSVAL) +
              EXPORT(*SRCFILE) SRCFILE(&LIB/&SRVSRC) +
              SRCMBR(R5SYSB) TEXT(&TEXT) +
              OPTION(*DUPPROC)
   SNDPGMMSG  MSG('Se ha creado el programa de servicio R5SYS.') +
              MSGTYPE(*COMP)
